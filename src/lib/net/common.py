...
from src.lib.net.dataset import Dataset
...
def add_train_args(parser):
  ...
  parser.add_argument("--network_type", type=str, required=True) # multiview, simnet
  parser.add_argument("--num_multiview", type=int, required=True) # 2,3,5
  parser.add_argument("--num_samples", type=int, required=True) # 43, 56
...
def get_model(hparams):
  if hparams.network_type == 'simnet':
    from simnet.lib.net.models import panoptic_net
    net_module = panoptic_net
    ...
    from model import multiview_net
    net_module = multiview_net
    net_attr = getattr(net_module, hparams.model_name)
    model = net_attr(hparams)
