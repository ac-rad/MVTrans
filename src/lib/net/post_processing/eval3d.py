...
from src.lib.net.post_processing import epnp, nms
from src.lib import occlusions
...
def measure_3d_iou(detections, gt_detections, known_depth=False, known_rotation=False):
  ...
  for detection in gt_detections:
    ...
    obj_labels.append(detection.obj_CAD)

  true_matches, pred_matches, overlaps, indices = compute_3d_matches(
      'single_class',
      np.array(gt_RTs),
      np.array(gt_scales),
  ...
  return true_matches, pred_matches, sorted_pred_scores, size_labels, ignore_labels, sorted_detections, overlaps


def measure_ADD(detections, gt_detections, CAD_list, EVAL_IOUS=np.arange(0.1, 0.8, 0.05)):
  gt_RTs = [detection.camera_T_object for detection in gt_detections]
  pred_RTs = [detection.camera_T_object for detection in detections]
  obj_labels = [detection.obj_CAD for detection in gt_detections]
  ...
  pred_scales=[np.diag(detection.scale_matrix[0:3, 0:3]) for detection in detections]
  gt_scales = [np.diag(detection.scale_matrix[0:3, 0:3]) for detection in gt_detections]
  pred_scores = [detection.score for detection in detections]
  pcds = [epnp._WORLD_T_POINTS @ detection.scale_matrix[0:3, 0:3] for detection in gt_detections]
  ...
  dis_threshold = np.arange(0.0, 0.105, 0.005)
  for i in range(len(obj_labels)):

    pcd = pcds[i]
    gt_pcd = pcd @ gt_RTs[i][:3, :3] + gt_RTs[i][:3, 3]
    
    try:
      pred_RT = [pred_RTs[int(j)] for j in true_matches[:, i]]
    except:
      return -1, -1, -1, -1, -1, -1

    for pred in pred_RT:
      pred_pcd=pcd @ pred[:3, :3] + pred[:3, 3]

      # ADD
      distance=gt_pcd - pred_pcd
      point_distance = np.linalg.norm(distance, axis=1)
      ADD_dist=np.mean(point_distance)
      ADD.append(ADD_dist)

      less2cm=(point_distance < 0.02).mean()
      less2cm_list.append(less2cm)

      ACC = [(point_distance < thres).mean() for thres in dis_threshold]
      AUC= metrics.auc(dis_threshold, ACC)
      AUC_list.append(AUC)

      # ADD-s
      nn_index = spatial.cKDTree(pred_pcd)
      nn_dists, _ = nn_index.query(gt_pcd, k=1)
      add_s_dist = nn_dists.mean()
      ADD_s.append(add_s_dist)

      less2cm_adds=(nn_dists < 0.02).mean()
      less2cm_adds_list.append(less2cm_adds)

      ACC_adds = [(nn_dists < thres).mean() for thres in dis_threshold]
      AUC_adds= metrics.auc(dis_threshold, ACC_adds)
      AUC_adds_list.append(AUC_adds)

  return ADD, ADD_s, AUC_list, less2cm_list, AUC_adds_list, less2cm_adds_list
